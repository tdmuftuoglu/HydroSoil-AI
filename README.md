...
This repository contains the analysis notebook, the input database, and the
tables, figures and model artefact it produces.
...
Several analyses reported in the manuscript are run by separate scripts in
`supplementary/` rather than by the main notebook, because they were added after
the main run and reuse its output rather than repeating it:

| Script | Analysis |
|---|---|
| `adams_sensitivity.py` | results under Adams' own fitted density constants |
| `orig_features.py` | the original descriptor set evaluated against the revised one |
| `dedup_test.py` | effect of exactly duplicated records on the row-wise result |
| `close_gaps.py` | duplicate accounting, performance by Ksat range, measured particle density, a final untouched hold-out, artefact checksums |
...
| File | Content |
|---|---|
| `HydroSoilAIv2.ipynb` | the main analysis pipeline |
| `supplementary/` | the additional analysis scripts listed above |
...
| `Fig1_sample.png` … `Fig8_screening_model.png` | the eight figures produced by the notebook, at 300 dpi |
...
The manuscript additionally contains figures that are not produced by the
notebook: a schematic of the analysis framework, a correlation heat map, a panel
comparing training-set SHAP attribution with out-of-fold importance, and
